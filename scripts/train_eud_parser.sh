...
# ENCODER_TYPE
test -z $6 && echo "Missing encoder type: 'transformer', 'g2g_transformer'"
test -z $6 && exit 1
ENCODER_TYPE=$6
...
for TBID in $TBIDS ; do
  ...
  for RANDOM_SEED in 756432; do
    ...
    for filepath in ${TB_DIR}/*/${TBID}-ud-train.conllu; do
      ...
      CONFIG=configs/${MODEL_TYPE}/eud_${ENCODER_TYPE}.jsonnet

      allennlp train $CONFIG \
      	-s logs/iwpt2021-submission-${DELEX_TYPE}/${TBID}-${FEATURE_TYPE}-${EDGE_MODEL_TYPE}-${ENCODER_TYPE}-${DELEX_TYPE}-${RANDOM_SEED}-${TIMESTAMP} \
